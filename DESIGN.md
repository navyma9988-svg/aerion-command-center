...
Hierarchy comes from tonal surfaces, a seven-percent border, faint top-edge highlight, and one soft shadow token. Token-derived luminous edges may reinforce an active control, focused navigation item, or operational status cell; they remain low-radius and low-opacity rather than becoming general neon decoration. Backdrop blur is limited to persistent chrome, map telemetry, sheets, and toasts. A vignette contains the Map hero. Nested cards and hard double borders are avoided.
...
Gold fill is reserved for the primary action. Secondary actions use tonal or hairline treatments. The shared `OpsButton` combines intent with emphasis and uses a calibrated 12px rectangular control shape instead of default pill geometry. A one-pixel edge lamp and brief hover/focus sheen make the control feel machined without changing its footprint; the lamp uses gold for brand intent and a semantic token only when the button itself performs that semantic operation. Brand outline controls and the active navigation item may carry one soft gold edge glow; status cells derive their edge light from the matching semantic token. Status colors communicate status, not general emphasis. Icon-only controls remain 44px and require an accessible name. Destructive actions remain separated and explicitly labeled.

### Navigation and data display

Mobile navigation uses the safe-area bottom dock; desktop uses the icon rail with accessible labels. Both use a quiet approach-light line and a contained active-route lamp rather than a generic floating-tab glow. Route transitions share the subtle slide/fade. Operational lists keep dividers quiet and numerical columns tabular. Queue and Live use continuous command boards: one outer instrument frame, shared dividers, and status-bound edge lamps replace stacks of visually unrelated cards. The Pulse hero uses the same aerial asset, surveyed geometry, and simulated flight tracks as the full Map in a compact command preview. Its four action metrics share one divided avionics console instead of reading as independent generic cards; thin illuminated rails encode each metric's existing semantic tone. The Map display switch is a two-option authored button group; one Controls trigger opens full-width overlay and traffic grids in a drawer instead of stacking controls above the canvas. The full display adds a north compass, map-derived scale, runway threshold markings, track history, heading vectors, and zoom-gated aircraft tags. Dense triage filters live in one command console with one visible option bank at a time instead of stacked pill carousels.
